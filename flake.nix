  ...
  outputs = { self }:
    let
      ...
      mlabsWelcomeText = ''
        ...
        There has been a decoupling between the downstream users and the library maintainers and between the 
        different downstream users which impose some very serious issues that are endangering the success for 
        certain projects and ultimately MLabs itself.
    ...
    in
    {
      templates = {
        ...
        mlabs-tooling = {
          path = ./mlabs-tooling;
          description = "A flake that provides everything necessary to set up a project within mlabs scope.";
          welcomeText = ''
            ${mlabsWelcomeText}
          '';
        };
